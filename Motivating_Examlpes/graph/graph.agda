...
record completeBound (subject : Set) (prop : subject -> Set) : Set₁  where
  field
    contains : subject -> Set
    complete : (s : subject) -> prop s -> contains s

record completeBoundDec (subject : Set) (prop : subject -> Set) : Set₁  where
  field
    boundProp : completeBound subject prop
    decProp : (s : subject) -> Relation.Nullary.Dec (prop s)



record vertbound {n : ℕ} (prop : Fin n -> Set) : Set₁  where
  field
    contains : Fin n -> Set
    complete : (s : Fin n) -> prop s -> contains s


--first pass, crush the problem with exhuastion?

-- why d's algoritm works
-- have a collection of nodes reachable in "the best way"
-- have a collection of potential paths such that the smallest is the next best guess
-- check the next best guess, either exend the potential paths, or remove from consideration


-- every path under the bound,
-- at most as good as any of the optimal paths, every path out of the collection returns and is usesless, or extends, if it is the best extended then the algorithm can repeat

record BestNodes {n : ℕ} {g : Graph {n}} {start : Fin n} {boundryCost : ℕ} : Set where
  get : (to : Fin n) -> {!all paths start to gte bound!}  ⊎ (Σ (Path start to) {!bestpath lte bnd!})
  -- completeness is implied

record Candidates {n : ℕ} {g : Graph {n}} {start : Fin n} {boundryCost : ℕ} : Set where
   best : {!unreachable!}  ⊎ Σ (Fin n) λ to → Σ (Path start to) λ p → Candidates {n} {g} {start} {cost p g} -- also gte bound




outE :  {n : ℕ} {g : Graph {n}} (from : Fin n) -> {!Collection !}  (Σ (Fin n) λ to → Path from to) --every path from other  will go through one, thus the cost must be bounded
outE = {!!}

step : {n : ℕ} {g : Graph {n}}
  -> {start : Fin n} (boundryCost : ℕ)
  -> (candidates : Candidates {n} {g} {start} {boundryCost} ) -- gte then the boundry cost
  -> {!unreachable!} ⊎ Σ ℕ (λ boundryCost'  → {!boundryCost ≤ boundryCost' !} × Candidates {n} {g} {start} {boundryCost'}) --{!boundryCost : ℕ . BestNodes, Candidates!}
step = {!!}



record stepdata {n : ℕ} {g : Graph {n}} {start : Fin n} {boundryCost : ℕ} : Set where
   bn : BestNodes {n} {g} {start} {boundryCost}
{-
step : {n : ℕ} {g : Graph {n}}
  -> {start : Fin n} (boundryCost : ℕ)
  -> (bestNodes : {!BestNodes!}) -- can lookup membership and paths lte the boundry cost
  -> (candidates : {!Candidates!}) -- gte then the boundry cost
  -> {!unreachable!} ⊎ {!boundryCost : ℕ . BestNodes, Candidates!}
step boundryCost bestNodes candidates with {!best!} candidates
... | inj₂ (boundryCost' , to , path, candidates' ) with {get} bestNodes  to 
... | inj₂  _  =  inj₂ {!(boundryCost' , bestNodes , candidates' )!} -- TODO need to PROVE that the BestNodes is complete over the larger bound
... | inj1 everything_worse = ...
...
best : {n : ℕ} {g : Graph {n}}
  ...
  -> {!unreachable!} ⊎ {!boundryCost : ℕ . to : Fin n. Path start to,  Candidates!}
best = {!!}
-}
